...
class NFTMarketplaceContract extends Contract {
    constructor(protocol, options = {}) {
        ...
        this.addSchema('credit_deposit_feature_value', {
        value: {
            $$strict: true,
            $$type: "object",
            user_address: { type: "is_hex" },
            amount: { type: "string", numeric: true },
            deposit_id: { type: "string", min: 4 }
        }
    });
    ...
    value: { 
        ...
        this.addFeature('admin_credit_feature', this.adminCreditFeature.bind(this));
    ...
    console.log("[CONTRATO] Feature 'notify_deposit_tap' ha sido ejecutada.");

    // Para una "feature", los datos están en `this.op.value`.
    const payload = this.op.value; 

    if (!payload) {
        console.error("[CONTRATO] ERROR: El payload (this.op.value) de la feature es nulo.");
        return; // Salir si no hay datos.
    }

    console.log("[CONTRATO] Payload recibido:", JSON.stringify(payload, null, 2));

    // Extraer los datos del payload.
    const { from_address: user_address, amount: depositAmountStr } = payload;

    if (!user_address || !depositAmountStr) {
        console.error("[CONTRATO] ERROR: 'from_address' o 'amount' no encontrados en el payload.");
        return;
    }

    console.log(`[CONTRATO] Procesando acreditación para ${user_address} por ${depositAmountStr}`);
    ...
    console.log(`[CONTRATO] ✅ ¡ÉXITO! Nuevo balance de ${user_address} es: ${newBalance.toString()}`);
}
    ...
    async adminCreditFeature() {
    // La "feature" recibe los datos en this.op, que tiene { key, value }
    const { key, value } = this.op;

    if (key === 'credit_deposit') {
        // Valida que los datos que llegaron son correctos
        if (!this.validateSchema('credit_deposit_feature_value', { value })) {
            console.error('[CONTRACT FEATURE] Datos de depósito inválidos.');
            return;
        }
        await this.processCreditDeposit(value);
    }
}

// 4. Esta es la lógica de negocio, ahora separada para mayor claridad.
async processCreditDeposit(data) {
    // El chequeo de admin aquí es simple y funciona porque es una feature.
    const admin = await this._get('admin');
    if (!admin || this.address.toLowerCase() !== admin.toLowerCase()) {
        throw new Error("Permission Denied: This feature can only be run by the admin peer.");
    }
    
    const { user_address, amount, deposit_id } = data;

    const alreadyProcessed = await this._get(`processed_deposits/${deposit_id}`);
    if (alreadyProcessed) {
        console.log(`[CONTRACT FEATURE] Depósito ID ${deposit_id} ya fue procesado. Omitiendo.`);
        return;
    }

    console.log(`[CONTRACT FEATURE] Acreditando ${this.protocol.fromBigIntString(amount, 18)} TAP a ${user_address.slice(0, 10)}...`);
    
    let userBalance = this.protocol.safeBigInt(await this._get(`internal_balances/${user_address}`, '0'));
    userBalance += this.protocol.safeBigInt(amount);
    await this.put(`internal_balances/${user_address}`, userBalance.toString());

    await this.put(`processed_deposits/${deposit_id}`, {
        tx_ref: this.tx, // Referencia al momento en que la feature se ejecutó
        processed_at: new Date().toISOString()
    });

    console.log(`[CONTRACT FEATURE] Acreditación para depósito ${deposit_id} completada.`);
}
