...
server.get('/api/collections', async (req, res) => {
    ...
    try {
        ...
        if (!file_id || !price) {
            return res.status(400).json({ error: 'file_id and price are required.' });
        }

        await peer.base.update();

        // Verificación 1: ¿Existe el NFT? (Sigue siendo una buena primera barrera)
        const metadata = await protocol.get(`file_meta/${file_id}`);
        if (!metadata) {
            console.error(`[Pre-Flight Failed] Attempted to list non-existent File ID: ${file_id}`);
            return res.status(404).json({
                error: 'NFT Not Found',
                details: `The File ID ${file_id} does not exist on the network.`
            });
        }

        // --- INICIO DE LA NUEVA VERIFICACIÓN DE PROPIEDAD ---
        // Verificación 2: ¿El wallet del servidor es el propietario de este NFT?
        const serverWalletAddress = peer.wallet.publicKey;
        const ownedNftIds = await protocol.get(`user_nfts/${serverWalletAddress}`) || [];
        
        if (!ownedNftIds.includes(file_id)) {
            // Si la lista de NFTs del servidor no incluye este file_id, no tiene permiso para venderlo.
            console.error(`[Pre-Flight Failed] Wallet ${serverWalletAddress.substring(0, 10)}... attempted to list NFT ${file_id.substring(0, 10)}... which it does not own.`);
            // Usamos el código 403 Forbidden, que es el estándar para errores de permisos.
            return res.status(403).json({
                error: 'Permission Denied',
                details: 'The server wallet is not the current owner of this NFT and cannot list it for sale.'
            });
        }
        // --- FIN DE LA NUEVA VERIFICACIÓN DE PROPIEDAD ---

        // Verificación 3: ¿Ya está a la venta? (Lógica que ya teníamos)
        const existingListing = await protocol.get(`listings/${file_id}`);
        if (existingListing) {
            console.warn(`[Pre-Flight Failed] NFT ${file_id} is already listed.`);
        ...
        // Si todas las verificaciones pasan, la transacción es segura.
        const command = { 
            op: 'listForSale', 
            file_id: file_id, 
        ...
        res.json({ success: true, message: 'List transaction submitted successfully.' });

    } catch (e) {
        console.error('[API /api/actions/list] UNEXPECTED ERROR:', e);
        res.status(500).json({ error: 'Failed to process listing due to an internal server error.', details: e.message });
    }
});
